...
\makepart{Introduction to Medical imaging}
...
\begin{center}
\vfill
\includegraphics[width=0.28\paperwidth]{Figures/glioma_example.png}\par
\vspace{0.6cm}
\textbf{Glioma?}\\
\textbf{ADEM?}\\
...
%                     \includegraphics[width=1\linewidth]{Figures/news2.png} % replace with news2.png
%                 };
...
%                     \includegraphics[width=1\linewidth]{Figures/news2.png}
%                 };
                
%                 % 3. Top Image (the most recent one you added)
%                 \node[anchor=center, rotate=-3] (img3) at (0.7,-1.1) {
%                     \includegraphics[width=1\linewidth]{Figures/news3.png}
%                 };
%             \end{tikzpicture}
%         \end{column}
%     \end{columns}
...
\begin{frame}{Trends of AI in Radiology}
    \begin{itemize}
        \item Geoffrey Hinton’s statement on radiology and AI in 2016: "It’s just obvious that within five years deep learning is going to do better than radiologists."
        \item A significant rise in AI-enabled medical devices cleared by FDA.
    \end{itemize}

    \begin{figure}
        \centering
        \includegraphics[width=0.75\textwidth, trim = 0 6em 0 0, clip]{Figures/ai-radio.png}
        % \caption{Sharp increase in research AI-enabled medical device starting around 2016}
    \end{figure}

    \vfill
    \footnotesize{Source: Food and Drug Administration (2025)}
\end{frame}
...
\node[phasebox, draw=modXray] at (p1) {
    \boximageWide{xray.png}
    {\raggedleft\tiny\color{white!70}Source: Livhospital \par} 
    \vspace{1em}
    \textbf{\HUGE X-Ray}\\[0.75cm]
...
\node[phasebox, draw=modCT] at (p2) {
    \boximageWide{ct.png}
    {\raggedleft\tiny\color{white!70}Source: Radiopaedia \par}
    \vspace{1em}
    \textbf{\HUGE CT}\\[0.75cm]
...
\node[phasebox, draw=modMRI] at (p3) {
    \boximageWide{mri.png}
    {\raggedleft\tiny\color{white!70}Source: AppliedRadiology \par}
    \vspace{1em}
    \textbf{\HUGE MRI}\\[0.75cm]
...
\begin{columns}[c,onlytextwidth]
    ...
    \centering
    \includegraphics[width=0.85\linewidth]{Figures/xray.png}
    \\ \vspace{0.5em}
...
\begin{columns}[c,onlytextwidth]
    ...
    \column{0.45\linewidth}
    \centering
    \includegraphics[width=0.85\linewidth]{Figures/ct.png}
    \\ \vspace{0.5em}
...
\makepart{COMMON TASKS IN MEDICAL IMAGE ANALYSIS}
...
\vfill
\begin{flushleft}
\tiny Source: \href{https://doi.org/10.1038/s41467-024-44824-z}{Wang et al., Nature Communications 2024}
\end{flushleft}
...
% --- 3. PHASE BOXES ---
\node[phasebox, draw=accTeal, above=2.5cm of p1] (top1) {\boximageCard{superResolution} \textbf{\Huge Preprocessing:} super resolution}; % https://mriquestions.com/super-resolution.html
\node[phasebox, draw=accTeal, below=2.5cm of p1] (bot1) {\boximageCard{registered-final} \textbf{\Huge Registration:}\\align coordinates};  %https://3dqlab.stanford.edu/image-registration/

\node[phasebox, draw=accGreen, above=2.5cm of p3] (top3) {\boximageCard{object-detection-medical-imaging} \textbf{\Huge Detection:}\\highlight elements};%https://www.imaios.com/en/resources/blog/introduction-to-the-most-common-deep-learning-architectures-for-object-detection-in-medical-imaging
\node[phasebox, draw=accGreen, below=2.5cm of p3] (bot3) {\boximageCard{segmentation} \textbf{\Huge Segmentation:}\\volume delineation};   %https://www.nature.com/articles/s41467-024-44824-z/figures/2

\node[phasebox, draw=accAmber, below=2.5cm of p2] (bot2) {\boximageCard{classification} \textbf{\Huge Classification:}\\malignant - benign  }; %https://www.sciencedirect.com/science/article/pii/S1877750324001170
\node[phasebox, draw=accAmber, above=2.5cm of p2] (top2) {\boximageCard{cell} \textbf{\Huge Classification:}\\identify cell types}; %https://pmc.ncbi.nlm.nih.gov/articles/PMC11810770/

\node[phasebox, draw=accCoral, above=2.5cm of p4] (top4) {\boximageCard{longitudinal} \textbf{\Huge Monitoring:}\\longitudinal study}; %https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0049072
\node[phasebox, draw=accCoral, below=2.5cm of p4] (bot4) {\boximageCard{outcome} \textbf{\Huge Prediction:}\\outcome analysis}; %https://news.mit.edu/2019/ai-model-atlas-patient-brain-analysis-1126

% --- 4. TITLES ---
\node[above=9.5cm of p1, font=\huge\bfseries] {PREPARATION};
\node[above=9.5cm of p2, font=\huge\bfseries] {FEATURE};
\node[above=9.5cm of p3, font=\huge\bfseries] {FEATURE};
...
% --- 3. PHASE BOXES ---
\node[phasebox, draw=accTeal, above=2.5cm of p1] (top1) {\boximageCard{superResolution} \textbf{\Huge Preprocessing:} super resolution};
\node[phasebox, draw=accTeal, below=2.5cm of p1] (bot1) {\boximageCard{registered-final} \textbf{\Huge Registration:}\\align coordinates}; 

\node[phasebox, draw=accGreen, above=2.5cm of p3] (top3) {\boximageCard{object-detection-medical-imaging} \textbf{\Huge Detection:}\\highlight elements};
\node[phasebox, draw=accGreen, below=2.5cm of p3] (bot3) {\boximageCard{segmentation} \textbf{\Huge Segmentation:}\\volume delineation};    

\node[phasebox, draw=accAmber, below=2.5cm of p2] (bot2) {\boximageCard{classification} \textbf{\Huge Classification:}\\malignant - benign};
\node[phasebox, draw=accAmber, above=2.5cm of p2] (top2) {\boximageCard{cell} \textbf{\Huge Classification:}\\identify cell types};

\node[phasebox, draw=accCoral, above=2.5cm of p4] (top4) {\boximageCard{longitudinal} \textbf{\Huge Monitoring:}\\longitudinal study};
\node[phasebox, draw=accCoral, below=2.5cm of p4] (bot4) {\boximageCard{outcome} \textbf{\Huge Prediction:}\\outcome analysis};
...
\begin{columns}[T,onlytextwidth]
    ...
    \uncover<1->{\begin{tcolorbox}[standard jigsaw, 
        ...
        colback=gray, colframe=teal!60!black, 
        colupper=white,
        title=\textbf{\color{teal!30}Data Scarcity},
        arc=3pt]
        \vspace{1em}
        \alert<1>{Modalities}:\,\,\,high-res 3D volumes but few patient samples. \\ [1em]
        \alert<2>{Long-tail}:\,\,\,common diseases dominate; rare cases are hard to obtain. \\ [2em]

        \hspace{2em}\begin{tikzpicture}[scale=0.55]
            \draw[->, thick, white!70] (0,0) -- (6,0);
            \draw[->, thick, white!70] (0,0) -- (0,4);
            
            \draw[ultra thick, accCoral, smooth] 
                (0.2,3.8) .. controls (0.5,0.5) and (2,0.2) .. (5.5,0.1);
            
            \fill[accCoral, opacity=0.3] 
                (0.2,3.8) .. controls (0.5,0.5) and (2,0.2) .. (5.5,0.1)
                -- (5.5,0) -- (0.2,0) -- cycle;

            \node[accCoral!80, font=\tiny] at (1.2,3.2) {Common};
    ...
    \uncover<3->{\begin{tcolorbox}[standard jigsaw, 
        opacityback=0.08, opacityframe=0.3, 
        colback=accCoral, colframe=gray!70!black,
        colupper=white,
        title=\textbf{\color{teal!30}Distribution Shift}, 
        arc=3pt]
        \vspace{1em}
         \alert<3>{Non-standard}:\,\,\,vendors (Siemens vs.\ GE), protocols differ. \\ [1em]
         \alert<4>{Subjective labels}:\,\,\,inter-observer variability. \\
    \end{tcolorbox}}

    \vspace{0.4em}

    % (2,2) Privacy Constraints
    \uncover<5->{\begin{tcolorbox}[standard jigsaw, 
        opacityback=0.08, opacityframe=0.3, 
        colback=teal, colframe=gray!70!black,
        colupper=white,
        ...
        arc=3pt]
        \vspace{1em}
        \alert<6>{De-identification risk} (e.g., facial reconstruction). \\[1em]
...
\vspace{5em}
    \centering
    \includegraphics[width=0.9\linewidth]{Figures/New_figure.png}
    \vspace{2em}
    {\tiny \textcolor{gray}{10.4018/978-1-5225-0140-4.ch004}}
...
\alert<5>{\textbf{Numerical Method}}
\begin{itemize}
...
% \begin{frame}{NEURAL NETWORKS: Data-Driven Template Matching}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{frame}{Evolution into NEURAL NETWORKS}
...
%       \item \alert<3>{NEURAL NETWORKS} reuse and generalize these ideas
%     \end{itemize}

%     \vspace{1.5em}
    
%     \uncover<4->{In Convolutional Neural Networks (NEURAL NETWORKS), the network \alert<4->{\textbf{learns}} these kernels.
%     } \\ [1em]

%     \uncover<5->{\begin{itemize}
%         \item Weights are optimized via backpropagation.
%         \item NEURAL NETWORKS did not replace classical vision $\rightarrow$ they learned it.
%     \end{itemize}}
...
\begin{itemize}
  ...
  \item<3-> \tikz[remember picture, baseline=(b3.base)]\node(b3){}; Requires \alert<3>{intense human intuition} (not end-to-end).
  
  \item<4-> \tikz[remember picture, baseline=(b4.base)]\node(b4){}; \alert<4>{Low generalizability} to complex objects or high intra-class variation.
\end{itemize}


\vspace{2em}

\textcolor{black}{{\textbf{Neural Networks}} generalize these ideas} \\[1em]
...
\uncover<2->{\alert{\textbf{Neural Networks}} generalize these ideas} \\[1em]
...
% -----------------------------
% \begin{frame}{Hierarchical Feature Learning in CNNs}
% \small
% \begin{columns}[T,onlytextwidth]

%     \column{0.42\linewidth}
%     When trained for classification, CNNs learn \alert<2->{\textbf{features in a hierarchy}} of increasing abstraction:

%     \vspace{0.4em}
%     \begin{itemize}\setlength\itemsep{0.25em}
%         \item \textbf{Early layers:} simple edges and colour gradients
%         \item \textbf{Middle layers:} textures, curves, and object parts
%         \item \textbf{Deep layers:} whole objects and scene-level concepts
%     \end{itemize}

%     \vspace{0.4em}
%     This mirrors the classical hand-crafted pipeline — but here the features are \emph{learned end-to-end} from data.

%     \vspace{0.5em}
%     {\tiny Zeiler \& Fergus, ECCV 2014}

%     \column{0.5\linewidth}
%     \centering
%     \vspace{0.2em}
%     \includegraphics[width=\linewidth]{zeiler_fig2.pdf}

%     \vspace{0.2em}
%     {\tiny Fig.\ 2: Top activations per feature map, Layers 1--5.}

% \end{columns}

% \vfill
% \begin{flushleft}
%     \tiny Source: \href{https://arxiv.org/abs/1311.2901}{Zeiler \& Fergus (2014), \textit{Visualizing and Understanding Convolutional Networks}}
% \end{flushleft}
% \end{frame}
...
When trained for classification, CNNs learn \alert<1->{\textbf{features in a hierarchy}} of increasing abstraction.

\vspace{0.5em}

% --- Bottom: Image ---
\begin{center}
    \includegraphics[width=1\linewidth, trim = 5em 10em 5em 10em, clip]{Overall.pdf}
...
\begin{columns}[T,onlytextwidth]
    ...
    \textbf{Early layers}\\
    \vspace{0.3em}
    Simple edges and colour gradients.
    ...
    %     \item Object parts
    % \end{itemize}
    Textures, curves, and object parts.
    ...
    % \begin{itemize}\setlength\itemsep{0.25em}
    %     \item Whole objects
    %     \item Scene-level concepts
    % \end{itemize}
    Whole objects and scene-level concepts.
...
\vspace{0.5em}
\begin{center}
    \includegraphics[width=0.75\linewidth]{zeiler_fig2.pdf}
...
% \begin{frame}{Evolution into NEURAL NETWORKS \& The Classical Ceiling}
...
%     \uncover<4->{\alert{\textbf{NEURAL NETWORKS}} reuse and generalize these ideas} \\[0.5em]
...
%         \end{itemize}
%         \vspace{0.2cm}
%         \textit{Classical CV tried to build this manually; NEURAL NETWORKS do it through Stochastic Gradient Descent.}
%     \end{column}
% \end{columns}
% \end{frame}
...
$\displaystyle W_x' = W_x - {\color{orange!80!black}\mathbf{\alpha}}\!\left(\frac{\partial \mathbf{Error}}{\partial W_x}\right)$
}
...
{Assume a given set of \textbf{\textcolor{yellow!60}{discrete labels}}, e.g.}\\[0.2cm]
\hspace{0.7cm}
\textcolor{gray!20}{\{husky, corgi, chihuahua, $\ldots$\}}
\end{minipage}
...
\begin{tabular}{ccc}
\textcolor{softblue}{$f($}
\includegraphics[height=0.9cm]{Figures/glioblastoma.png}
\textcolor{softblue}{$)$}
&
\textcolor{softblue}{$=$}
&
\textcolor{yellow!60}{``Glioblastoma''}
\\[0.35cm]

\textcolor{softblue}{$f($}
\includegraphics[height=1.05cm]{Figures/menigioma.png}
\textcolor{softblue}{$)$}
&
\textcolor{softblue}{$=$}
&
\textcolor{yellow!60}{``Meningioma''}
\\[0.35cm]

\textcolor{softblue}{$f($}
\includegraphics[height=1.05cm]{Figures/brain metastis.png}

\textcolor{softblue}{$)$}
&
\textcolor{softblue}{$=$}
&
\textcolor{yellow!60}{``Metastasis''}
\end{tabular}

\vfill

{\small \textcolor{gray!55}{Goal: learn a function that maps an image to one label from a fixed label set.}}
...
%--------------------------------------------
\begin{frame}{Image Classification Model}
\small
...
%     \column{0.42\linewidth}
    \centering
    \includegraphics[width=0.9\linewidth]{Figures/cnn_model.png}
...
\vfill
\begin{flushleft}
\tiny Source: \href{https://www.ibm.com/think/topics/image-classification}{IBM.com}
\end{flushleft}
...
\begin{frame}{\textcolor{accent}{Activation Functions}}
...
\begin{itemize}
    \item $\textit{Output}=\dfrac{1}{1+e^{-\textit{x}}}$
    \item Nonlinear
\end{itemize}
...
\addplot[thick, cyan, smooth] {1/(1+exp(-x))};
\end{axis}
\end{tikzpicture}
...
\begin{itemize}
    \item $\textit{Output}=\max(0,x)$
    \item Efficient
    \item Compositions are nonlinear
    \item Bound: $[0,\infty)$
\end{itemize}
...
\addplot[thick, cyan, domain=-5:0] {0};
\addplot[thick, cyan, domain=0:5] {x};
\end{axis}
\end{tikzpicture}
...
% \begin{frame}{\textcolor{accent}{Activation Functions Comparison}}

% \scriptsize
% \setlength{\tabcolsep}{4pt}

% \begin{tabular}{|p{1.5cm}|p{2cm}|p{3cm}|p{4cm}|p{4cm}|}
% \hline
% \textbf{Used in} & \textbf{Activation} & \textbf{Details} & \textbf{Pros} & \textbf{Cons} \\
% \hline
...
% Range: $[0,1]$ &
% \begin{itemize}
% \item Smooth output between 0 and 1
...
% \item Helps faster convergence
% \end{itemize}
% &
% \begin{itemize}
% \item Still suffers vanishing gradient
% \end{itemize}
% \\
% \hline
...
% Range: $[0,\infty)$ &
% \begin{itemize}
% \item Avoids saturation (positive side)
...
\begin{tabular}{|p{1.5cm}|p{1.5cm}|p{2.5cm}|p{3cm}|p{3cm}|}
\hline
\textbf{Used in} & \textbf{Activation} & \textbf{Details} & \textbf{Pros} & \textbf{Cons} \\
\hline
...
&
\begin{itemize}
\item Avoids saturation on positive side
\item Computationally efficient
\item Faster convergence
\end{itemize}
&
...
% \begin{frame}{\textcolor{accent}{Softmax Classifier (Multinomial Logistic Regression)}}

% \small
% \centering

% {\Large Want to interpret raw classifier scores as probabilities}
...
% \begin{tikzpicture}[
%     font=\small,
%     box/.style={draw, thick, minimum width=1.5cm, minimum height=3.0cm},
%     arrow/.style={->, thick, white!75},
%     note/.style={align=center}
% ]

% % labels
% \node[align=right] at (-4.6,1.1) {malamute};
% \node[align=right] at (-4.6,0) {corgi};
% \node[align=right] at (-4.6,-1.1) {chihuahua};

% % logits
% \node[box, blue, text=white] (logits) at (-3,0) {};
% \node[text=white] at (-3,1.1) {\Large 3.2};
% \node[text=white] at (-3,0) {\Large 5.1};
% \node[text=white] at (-3,-1.1) {\Large -1.7};
% \node[note, blue] at (-3,-2.4) {Unnormalized log-\\probabilities / logits};

% % exp
% \node[box, red, text=white] (expbox) at (-0.5,0) {};
% \node[text=white] at (-0.5,1.1) {\Large 24.5};
% \node[text=white] at (-0.5,0) {\Large 164.0};
% \node[text=white] at (-0.5,-1.1) {\Large 0.18};
% \node[note, red] at (-0.5,2.3) {Probabilities\\must be $\geq 0$};
% \node[note, red] at (-0.5,-2.45) {unnormalized\\probabilities};

% % probabilities
% \node[box, green!70!black, text=white] (probbox) at (2.3,0) {};
% \node[text=white] at (2.3,1.1) {\Large 0.13};
% \node[text=white] at (2.3,0) {\Large 0.87};
% \node[text=white] at (2.3,-1.1) {\Large 0.00};
% \node[note, green!70!black] at (2.3,2.3) {Probabilities\\must sum to 1};
% \node[note, green!70!black] at (2.3,-2.3) {probabilities};

% % correct probs
% \node[box, violet, text=white] (correct) at (5.9,0) {};
% \node[text=white] at (5.9,1.1) {\Large 0.00};
% \node[text=white] at (5.9,0) {\Large 1.00};
% \node[text=white] at (5.9,-1.1) {\Large 0.00};
% \node[note, violet] at (5.9,-2.3) {Correct\\probs};

% % arrows and labels
% \draw[arrow] (logits.east) -- (expbox.west)
%     node[midway, above, text=white] {exp};

% \draw[arrow] (expbox.east) -- (probbox.west)
%     node[midway, above, text=white] {normalize};

% \draw[arrow] (probbox.east) -- ++(0.9,0)
%     node[midway, above, text=white] {compare};

% \draw[arrow] (correct.west) -- ++(-0.9,0);

% \node[text=white, align=center] at (5.05,2.15) {Softmax\\Function};
...
{\Large Interpret raw classifier scores as probabilities}

\vspace{0.2cm}

\[
\textcolor{white}{s = f(x_i; W)}
\qquad
\textcolor{white}{P(Y = k \mid X = x_i)
=
\dfrac{e^{s_k}}{\sum_j e^{s_j}}}
\qquad
L_i = -\log P(Y = y_i \mid X = x_i)
...
\begin{tikzpicture}[
    font=\small,
    box/.style={draw, thick, minimum width=1.45cm, minimum height=3.0cm},
    arrow/.style={->, thick, white!75},
    note/.style={align=center}
]

% image placeholder
\node[minimum width=2.1cm, minimum height=1.45cm, align=center] 
    at (-3.35,2.25) {\includegraphics[width = 1cm]{Figures/menigioma.png}};

% labels
\node[align=right, text=white] at (-5.0,1.1) {Glioblastoma};
\node[align=right, text=white] at (-5.0,0) {Meningioma};
\node[align=right, text=white] at (-5.0,-1.1) {Metastasis};

% logits
\node[box, blue, text=white] (logits) at (-3.35,0) {};
\node[text=white] at (-3.35,1.1) {\Large 3.2};
\node[text=white] at (-3.35,0) {\Large 5.1};
\node[text=white] at (-3.35,-1.1) {\Large -1.7};
\node[note, blue] at (-3.35,-2.4) {Unnormalized log-\\probabilities / logits};

% exp values
\node[box, red, text=white] (expbox) at (-0.8,0) {};
\node[text=white] at (-0.8,1.1) {\Large 24.5};
\node[text=white] at (-0.8,0) {\Large 164.0};
\node[text=white] at (-0.8,-1.1) {\Large 0.18};
\node[note, red] at (-0.8,2.3) {Probabilities\\must be $\geq 0$};
\node[note, red] at (-0.8,-2.45) {unnormalized\\probabilities};

% probabilities
\node[box, green!70!black, text=white] (probbox) at (2.0,0) {};
\node[text=white] at (2.0,1.1) {\Large 0.13};
\node[text=white] at (2.0,0) {\Large 0.87};
\node[text=white] at (2.0,-1.1) {\Large 0.00};
\node[note, green!70!black] at (2.0,2.3) {Probabilities\\must sum to 1};
\node[note, green!70!black] at (2.0,-2.3) {probabilities};

% correct labels
\node[box, white, text=white] (correct) at (5.45,0) {};
\node[text=white] at (5.45,1.1) {\Large 1.00};
\node[text=white] at (5.45,0) {\Large 0.00};
\node[text=white] at (5.45,-1.1) {\Large 0.00};
\node[note, white] at (5.45,-2.3) {Correct\\probs};
...
\draw[arrow] (expbox.east) -- (probbox.west)
    node[midway, above, text=white] {normaize};

\draw[arrow] (probbox.east) -- ++(0.8,0)
    node[midway, above, xshift=15pt, text=white] {compare};

\draw[arrow] (correct.west) -- ++(-0.8,0);

\node[text=white, align=center] at (5.5,2.25) {Cross\\ entropy loss};
